...
`libusb-1.0.dll.a` from the matching
[libusb 1.0.29](https://github.com/libusb/libusb/releases/tag/v1.0.29) build
into `third_party/libusb/lib/`: `MinGW64` for x64, or
`MinGW-llvm-aarch64` for ARM64. The receiver needs the WinUSB driver, which
...
`master` publishes one per system (Windows x64 and ARM64, Linux x64 and
ARM64, macOS ARM64) as a release, tagged with the version in
`CMakeLists.txt` and the commit.
...
|---|---|---|
| [libusb](https://libusb.info/) 1.0.29 | All communication with the USB receiver | LGPL-2.1-or-later |
